...
選手一覧・選手詳細・ランキング・通算成績・節別結果・比較・BEYONDのページ構成（ナビは6項目、
選手詳細は各ページの選手名リンク経由）で、単にデータを溜めるだけでなく表示まで一体になっています。

svlabo.jp由来のうち、節別詳細（`battle_details.csv`）はカテゴリ巡回で毎日自動取得します。
ランクマッチのリーダーボード（`svlabo_leaderboards.csv`）だけは手動運用のままです。
...
│   ├── scrape_schedule.py      シーズン全体の日程（schedule.csv）を公式サイトから取得（毎日自動。詳細は「日程について」参照）
...
│   │                             （毎日自動。カテゴリページを巡回して新しい節の記事を自動発見する。
│   │                              未実施の記事はスキップする。詳細は「節別詳細について」参照）
...
│   ├── schedule.csv            シーズン全体の日程（毎日自動更新。14節56ROUND分の日付・時刻・
│   │                             対戦カード・スコア。詳細は「日程について」参照）
│   ├── battle_details.csv      svlabo.jpの節別記事から取り込んだ試合詳細（毎日自動更新。
│   │                             使用/未使用クラス・配信URL付き。詳細は「節別詳細について」参照）
...
└── site/                        表示用ページ。ページごとにファイルを分けており、今後ページを
    ...
    │                             クラスアイコン表示など）
    ├── index.html               チーム一覧（トップページ）。上部に「次の節」パネル、その下にチーム順位表。
    │                             チームをクリックすると選手カードが展開されるアコーディオン形式
    ├── player.html              選手詳細（写真・現在値・X/YouTubeへのリンク・折れ線グラフ、
    │                             ?name=選手名 で表示選手を切替）
    ├── ranking.html             ランキング（指標×期間/しきい値で選手を順位付け表示。全選手を常に表示。
    │                             PS公式戦の勝利数/獲得ポイントもここから見られる）
    ├── rounds.html              通算成績（全選手のリーダーボード＋個人の対戦履歴。
    │                             ?name=選手名で個人成績を直接表示。旧URL rounds.html?view=sections は
    │                             sections.htmlへ転送する）
    ├── sections.html            節別結果（節ごとのROUND/BATTLE詳細・使用/未使用クラス・配信リンク。
    │                             以前はrounds.html内のページ内タブだったが、利用頻度が高いためナビ第1階層に独立）
...
3. `data/history.csv` / `data/match_results.csv` / `data/player_images.json` /
   `data/snapshots/` / `data/result.json` / `data/battle_details.csv` / `data/schedule.csv` には一切触れない・
   コピー対象に含めない
   （`battle_details.csv`は以前は手動運用でしたが、カテゴリ巡回による自動取得に変わったため
   **Actionsが日々書き込む実データ側に移りました**。古いzipの内容で上書きすると取り込み済みの節が消えます）
4. コミット→push
...
## なぜランクマッチのリーダーボードだけ手動なのか

svlabo.jp由来のデータのうち、節別詳細（`battle_details.csv`）はカテゴリページを巡回して
毎日自動取得しています。手動運用のままなのはランクマッチのリーダーボード
（`svlabo_leaderboards.csv`、`scrape_svlabo.py`）だけです。こちらは当初、毎日自動取得→
毎月1日のみ自動取得、と段階的に自動化しようとしていましたが、以下の理由で自動化対象から外しました。
...
### 対戦成績について（match_results.csv・battle_details.csv / rounds.html・sections.html）

対戦まわりは主語が違う2つのページに分けています。

- **通算成績**（`rounds.html`）… 主語は「選手」。誰が何勝何敗で通算成績はどうか。
  `?name=選手名` でその選手個人の対戦履歴を表示します。データ元は`match_results.csv`。
- **節別結果**（`sections.html`）… 主語は「試合」。第N節のROUND1で誰がどのクラスを使い、
  何を登録して使わなかったか、配信のどこから見られるか。データ元は`battle_details.csv`。

一時期この2つを`rounds.html`のページ内タブ（「通算成績」「節別に見る」ボタン）にまとめて
いましたが、節別結果はナビから2階層下に埋もれる割に利用頻度が高かったため、ナビの第1階層に
独立させました。旧URL`rounds.html?view=sections`は`sections.html`へ自動転送します。

さらに古い`matches.html`は、旧URL（`matches.html`、`matches.html?name=選手名`）を共有・
ブックマークしている人のために、クエリ文字列を保ったまま`rounds.html`へ転送するだけの
リダイレクトスタブとして残してあります。
...
|---|---|
| section | 節番号（1〜14） |
| half | `前半` / `後半`（第3節のように1日開催で分かれていない節は空） |
| round_no | その節（前半/後半）の中でのROUND番号 |
| battle_no | そのROUNDの中でのBATTLE番号 |
| round | 試合の開催日（例: `2026-07-22`） |
...
重複排除のキーは `(section, half, round_no, battle_no, player_name)` です（`date`は含めません）。
同じ消化済み試合は毎日のスクレイピングで繰り返し観測されるので、`date`をキーに含めると
実行のたびに「別の試合」として重複記録されてしまうためです。

> 以前はキーが `(round, player_name)` で、`round`は試合日時の文字列でした。2026-08-27に
> 公式サイトのリニューアルでパースが崩れた際、`round`が`match_14`のようなダミー値、
> `player_name`がクラス名になった行が生成され、**毎回新しいキーになるため既存行を上書きせず
> 153行そのまま積み上がりました**。節・ROUND・BATTLE番号をキーにすることで、同じバトルは
> 必ず同じキーになり増殖しません。また`section`が数字でない行・`result`が`WIN`/`LOSE`でない行は
> マージ時に落とすので、過去に混入したゴミ行も自動的に消えます。
...
これを`data/battle_details.csv`に取り込み、`sections.html`（ナビの「節別結果」）で表示しています。使ったクラスは通常表示、登録していたが使わなかったクラスは薄く表示することで、
一目で区別できるようにしています。

**取得方法（毎日自動）**: 以前はURL（`blog-entry-XXXX.html`）に規則性が無く新記事を自動発見できない
として手動運用していましたが、カテゴリページ

    https://svlabo.jp/blog-category-61.html   （プレミアシリーズ）

がJS描画ではない素のHTMLで、記事タイトルとURLがそのまま並んでいることを確認したので自動化しました。
タイトルの命名が一貫しているため、節番号と前半/後半をタイトルから取り出せます。
...
別記事も混ざりますが、**「試合詳細結果」という語**で絞り込めます。サイドバーの「最新記事」欄にも
同じリンクが出るので記事IDで重複排除しています。既にCSVに入っている節は再取得しません。

**未実施の記事を除外する仕組み（重要）**: svlabo.jpは翌日以降の対戦分の記事を先に公開します。
その時点では対戦カード・出場選手・登録クラスは埋まっていますが、結果は空です。
...
**「配信URL (時間指定)」のリンクは未実施の記事にも出る**ので、リンクの有無では判定できません。
判定には次の2つを使っています。

1. 各バトルが実施済みか … `winlose` が `WIN`/`LOSE` で、`use1`/`use2` が `"0"` でない
2. 各ROUNDにチームバトルの行があるか … チーム戦は必ず1戦以上行われるため、決着したROUNDには
   `pro1`/`pro2` が空の行（＝チームバトル）が必ず含まれる。未実施の記事は個人戦3枠しか
   用意されておらず、この行が存在しない

1つでも未実施のバトルがある記事、またはチームバトル行が無いROUNDを含む記事は、その記事を
**丸ごとスキップ**します（中途半端な状態で取り込まないため）。試合中に巡回して前半だけ埋まった
状態を掴んでも、翌日の巡回で完全な状態を取り込み直せます。

**手動実行**: URLを直接指定して取り込むこともできます（過去分の再取得やデバッグ用）。

```
cd scripts
python scrape_svlabo_battle_details.py                       # カテゴリ巡回（自動実行と同じ）
...
`<half>`（`前半戦`/`後半戦`）を省略すると「1記事に4ROUND入っている」前提の自動判定になります。
巡回モードではタイトルから判定するのでこの指定は不要です。
...
別なので、共通のキーがありません。そのため`rounds.html`の個人成績表（`?name=選手名`）では「対戦した2チームの組み合わせ＋選手名」
...
（バグではなく、単にその節のsvlabo.jp記事がまだ公開されていない/未実施として除外されているだけです。
記事が公開されて結果が埋まれば、翌日の巡回で自動的に取り込まれます）。

## 日程について（schedule.csv / トップページの「次の節」）

公式サイトのSCHEDULE & RESULTSページには、レギュラーシーズン14節分の日程が最初から全部
載っています（対戦カードも確定済み）。これを`scrape_schedule.py`が毎日取得して
`data/schedule.csv`に保存します。一度CSVに入れた日程は、公式の構成が変わっても残ります。
...
| date | 開催日（YYYY-MM-DD） |
| weekday | 曜日（SAT/SUN/WEDなど公式表記のまま） |
| time | 開始時刻（HH:MM）。**未発表なら空** |
| round_no | その日の中でのROUND番号 |
| team1 / team2 | 対戦する2チームのteam_tag |
| score1 / score2 | バトル勝ち数。未消化なら空 |

**開催時刻は消化済みの節にしか入っていません**（公式が直前に発表する運用）。日付と曜日は
14節分すべて入っています。時刻が空の行は「日付は決まっているが時刻未定」を意味します。

### なぜカレンダーにしなかったか

当初カレンダー形式を検討しましたが、日程の実データを見てやめました。シーズンは
2026/07/11〜2027/01/24の約7ヶ月ですが、**開催日は23日しかありません**。
...
月グリッドにすると30マス中3マス程度しか埋まらず、12月は1日だけです。月送りを7回押しながら
ほぼ空白のマスを眺めることになり、スマホでは7列グリッドの負担も大きい。そのため
「次の節だけを大きく出す」形にしました（`index.html`の`nextSectionHTML()`）。

### 「次の節」の定義

**まだ消化していない試合が残っている、一番小さい節**です（`common.js`の`findNextSection()`）。
節の中で前半戦だけ終わって後半戦が未消化、という状態があるため「次の開催日」ではなく
「節単位でまだ終わっていないもの」を採っています。その節の中で既に終わった日は
薄く表示して「終了」と付け、未消化の日を主役として出します。全節消化済みなら
「全日程終了」と表示します。

## スマホのナビについて

ナビが6項目になり、スマホでは横一列に収まりません。以前は横スクロールにしていましたが、
スクロールできることが気付かれにくく現在地も分かりにくいため、**画面幅768px以下だけ
折りたたみメニュー（ハンバーガー）**に変えました。

チェックボックスとlabelで作っているのでJavaScriptは不要です。769px以上ではチェックボックスと
labelを`display:none`にするだけで従来のタブ表示に戻るため、**PC表示は一切変わりません**。

> `<details>`/`<summary>`でも作れますが、その場合PCでもナビ本体が折りたたまれてしまい、
> UAスタイルを上書きする必要が出るため採用しませんでした。

## 公式サイトのリニューアルについて（2026-08-27）

`ps.shadowverse-wb.com` が全面リニューアル（Svelte製に作り直し）され、DOMもテキストの並びも
全部変わりました。旧クラス名（`.results__game` / `.results__team-name` / `.results__score-num` など）は
1件もヒットしなくなり、これに依存していた3つのスクリプトが同時に壊れました。

| スクリプト | 壊れ方 | 対応 |
|---|---|---|
| `scrape_ps_results.py` | モーダルの並び順決め打ちが崩れ、列が1つずつズレた行を153行書き込んだ | DOM構造ベースに全面書き換え＋検算追加 |
| `scrape_result.py` | セレクタが全滅。ただし検算が効いて既存result.jsonは保護された（取得は停止） | 新セレクタに更新 |
| `scrape_player_images.py` | 画像のホスト・パスが変わり、ページもSPA化してrequestsでは読めなくなった | Playwright描画＋新URL規則に更新 |

新しいDOMではクラス名に `svelte-xxxxx` というハッシュが付きます。**これはビルドのたびに
変わるのでセレクタに使ってはいけません**。各スクリプトはハッシュを含まない部分
（`battle-card`, `-score-value`, `-team-score-name` など）だけを使っています。

このリニューアルで公式サイト側にも「残り登録デッキ」とデッキリストURLが載るようになりました
（svlabo.jpから手動で取り込んでいる`battle_details.csv`と同種の情報）。現時点では取り込んでいません。
