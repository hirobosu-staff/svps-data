...
順位表・選手一覧・選手詳細・通算成績・節別結果・比較・BEYONDのページ構成（ナビは6項目、
...
│   │                             compare.htmlの「BEYOND回数」列とbeyond.htmlが参照する）
...
└── site/                        表示用ページ。ページごとにファイルを分けており、今後ページを
    ...
    │                             クラスアイコン表示など）
    ├── index.html               順位表（トップページ）。上部に「次の節」パネル、その下にチーム順位表。
    │                             チームをクリックすると選手カードが展開されるアコーディオン形式
    ├── players.html             選手一覧（37人をフラットに表示。チーム絞り込みチップが主、名前検索が従。
    │                             カードから選手詳細へ飛ぶ。詳細は「選手一覧について」参照）
    ├── player.html              選手詳細（写真・現在値・X/YouTubeへのリンク・折れ線グラフ、
    │                             ?name=選手名 で表示選手を切替）
    ├── ranking.html             旧ランキングページ。比較の「表で比較」と役割が重複していたため廃止し、
    │                             compare.htmlへ転送するだけのリダイレクトスタブになっている
    ...
    │                             「表で比較」＝全選手を12列のソート可能な一覧表にまとめたもの。
    │                             期間セレクタと直近30日の増減列を持つ。詳細は「比較の表について」参照）
...
`compare.html`の「BEYOND回数」列はこの対応表と`normalizeName()`
...
なおPS公式戦の戦績は`compare.html`の「表で比較」の「戦績」列からも一覧で見られます。
...
## 選手一覧について（players.html）

選手個別ページ（`player.html`）への入口です。

もともとトップページ（`index.html`）がこの役割を兼ねていましたが、あちらはチーム順位表が主役で、
選手カードはアコーディオンの中に畳まれており**既定で開いているのは1チームだけ**でした。
「選手一覧」という名前なのに37人中4〜5人しか見えておらず、個別ページへ辿り着きにくい状態だったため、
**順位表（`index.html`）と選手一覧（`players.html`）に役割を分けました**。

絞り込みは**チームが主、名前検索が従**です。実際の使い方として「CRの選手を見たい」の方が
「名前を打ち込んで探す」より多いためです。検索欄は選手名だけでなくチーム名・チームタグにも
一致するので、「レバンガ」でも「zeta」でも絞り込めます。

選手カードは順位表と同じ`.player-card`をそのまま使っており、見た目の作りは変えていません。
全員表示のときだけチームごとに見出しで区切り、絞り込み中・検索中は見出しを挟まず単純に並べます。

`?team=CR` `?q=名前` のクエリを付けて、絞った状態のURLを直接開くこともできます。

## 比較の表について（compare.html の「表で比較」）

全選手を12列のソート可能な一覧表にまとめたページです。旧`ranking.html`（1指標ずつ順位を出す）は
このページと役割が完全に重複していたため廃止し、あちらにしか無かった機能をこちらに移しました。

| 列 | 内容 |
|---|---|
| チーム / 名前 | 名前から選手詳細へ飛べる |
| フォロワー | Xフォロワーの現在値 |
| フォロワー30日 | 30日前との増減（実数と率） |
| 登録者 | YouTube登録者の現在値 |
| 登録者30日 | 30日前との増減（実数と率） |
| 戦績 | PS公式戦の通算勝敗 |
| 直近1ヶ月の配信 | 直近2期間（およそ1ヶ月）の配信時間合計 |
| 配信時間 / 動画本数 | 期間セレクタで選んだ期間の値（既定は通算） |
| 最高順位 / BEYOND回数 | svlabo.jpのランクマッチデータ由来 |

**なぜ「直近」の列を足したか**: 現在値だけだと、もともと母数の大きい選手が常に上位に来るだけで
「最近伸びているか」が分かりません。通算配信時間も同様に「昔たくさん配信した人」が有利になります。
そこで直近の増減・活動量を別列として持たせています。

**増減は実数と率の両方を出しています**。実数だけだと母数の大きい選手が常に上位になり、
率だけだと母数の小さい選手が跳ねるためです（実際、増加数トップともっちゃま、伸び率トップの
つきあかめんは別人です）。緑が増加、赤が減少です。

**30日比の仕組み**: `history.csv`は`followers`/`youtube_subscribers`を毎日`current`として
記録しているので、最新の観測日から30日前に最も近い観測を探して差を取ります（`changeOverDays()`）。
取得が落ちた日があってもいいように前後3日まで許容し、それでも見つからなければ「-」にします。
現時点では37人全員でフォロワーの30日比が出ています（登録者はYouTubeチャンネルが無いChappyのみ「-」）。
なお`history.csv`は2026-07-26開始なので、30日比が出せるのは2026-08-25以降です。

**直近1ヶ月の配信時間**: 配信時間は隔週期間ごとの値なので「今月」という単位がデータ側にありません。
1期間（約2週間）だけだと「たまたまその2週間配信しなかった」だけで0になり指標として不安定なため、
直近2期間の合計（およそ1ヶ月）で見ています（`recentPeriodTotal()`）。

**期間セレクタ**: 配信時間・動画本数を隔週期間ごとに切り替えて見られます。旧`ranking.html`の
期間絞り込みを移したものです。既定は「通算」です。
